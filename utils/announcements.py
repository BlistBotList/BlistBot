...
async def _get_unique_id(ctx, table_type: str, bot_user_id: int) -> Optional[int]:
    queries = {
...
async def _from_unique_id(ctx, table_type: str, unique_id: int) -> Optional[Union[Bot, Author]]:
    queries = {
...
class Announcement:
    ...
    async def get_author_object(self, ctx: Context) -> Optional[Author]:
        """Returns an AnnouncementCreator object."""
        return await _from_unique_id(ctx, "USER", self.author_id)  # type: ignore

    async def get_bot_object(self, ctx: Context) -> Optional[Bot]:
        """Returns an AnnouncementBot object."""
        return await _from_unique_id(ctx, "BOT", self.bot_id)  # type: ignore

    @classmethod
    async def insert(cls, ctx, announcement: str, bot_id: int, pinned: bool = False) -> Union[Announcement, str]:
        ...
        bot_id = await _get_unique_id(ctx, "BOT", bot_id)  # type: ignore
        author_id = await _get_unique_id(ctx, "USER", ctx.author.id)
        time = discord.utils.utcnow()
        try:
            return_columns = await ctx.bot.pool.fetchrow(
                announcement_query, bot_id, author_id, announcement, time, pinned
            )
    ...
    @classmethod
    async def from_unique_id(cls, ctx, unique_id: int) -> Optional[Announcement]:
        """Gets an announcement via the unique_id and returns an Announement object,
        if successful, else, will return None.
        """
        announcement_fetched = await ctx.bot.pool.fetchrow(
            "SELECT * FROM main_site_announcement WHERE unique_id = $1", int(unique_id)
        )
    ...
    @classmethod
    async def fetch_bot_announcements(
        cls, ctx, bot_id: int, limit=None, *, oldest: bool = False, is_unique: bool = False
    ) -> List[Announcement]:
        """
    ...
    async def delete(self, ctx, bot_id: int) -> Optional[Announcement]:
        ...
        if announcement_bot.id == bot_id:  # type: ignore
            bot_id = await _get_unique_id(ctx, "BOT", bot_id)  # type: ignore
    ...
    async def edit(self, ctx, new_content: str, bot_id: int) -> Optional[Announcement]:
