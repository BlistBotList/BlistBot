...
import pytz
from utils.buttons import ApprovalFeedbackView
...
class Events(commands.Cog):
    ...
    async def update_staff_embed(self, guild: discord.Guild) -> None:
        web_mods_query = await self.bot.mod_pool.fetch(
            "SELECT userid, country_code FROM staff WHERE rank = $1", "Website Moderator"
        )
        senior_web_mod_query = await self.bot.mod_pool.fetch(
            "SELECT userid, country_code FROM staff WHERE rank = $1", "Senior Website Moderator"
        )
        ...
        senior_admins_query = await self.bot.mod_pool.fetch(
            "SELECT userid, country_code FROM staff WHERE rank = $1", "Senior Administrator"
        )

        def get_string(user: Record) -> str:
            mem = guild.get_member(int(user["userid"]))
            assert mem is not None  # linter stfu
            return f"{mem.mention} :flag_{str(user['country_code']).lower()}:"

        senior_administrators = [get_string(user) for user in senior_admins_query]
        administrators = [get_string(user) for user in admins_query]
        senior_website_moderators = [get_string(user) for user in senior_web_mod_query]
        website_moderators = [get_string(user) for user in web_mods_query]
        ...
        embed.add_field(
            name="> Senior Website Moderators", value="\n".join(senior_website_moderators) or "None", inline=False
        )
        ...
        assert isinstance(channel, discord.TextChannel)  # linter stfu
    ...
    async def on_command_error(self, ctx, error):
        ...
        em = discord.Embed(title="Bot Error:", description=f"```py\n{error}\n```", color=discord.Color.blurple())
        await ctx.send(
            embed=discord.Embed(
                description=f"Something went wrong... {type(error).__name__}", color=discord.Color.red()
            )
        )
        await self.error_webhook.send(embed=em)
...
""",
        ...
        if message.guild == self.bot.main_guild:
            ...
            if user:
                ...
                leveling_user = await self.bot.pool.fetch(
                    "SELECT * FROM main_site_leveling WHERE user_id = $1", user["unique_id"]
                )
                ...
                if (
                    ...
                    await self.bot.pool.execute(
                        "UPDATE main_site_leveling SET xp = xp + $1 WHERE user_id = $2", xp, user["unique_id"]
                    )
    ...
    async def lvl_up(self, db_user, message):
        ...
        if xp >= lvl * 50:
            await self.bot.pool.execute(
                "UPDATE main_site_leveling SET level = level + 1, xp = $1 WHERE user_id = $2", 0, db_user["unique_id"]
            )
        ...
        if member.guild == self.bot.verification_guild:
            if member.bot:
                ...
                if not bot:
                    ...
                    message = await channel.send(embed=embed, view=ApprovalFeedbackView(self.bot, member))
                    await message.pin()
    ...
    async def on_member_update(self, before, after):
        ...
        if list(set_difference2) != []:
            ...
            await self.bot.mod_pool.execute(
                "UPDATE staff SET rank = $1 WHERE userid = $2", new_roles[0].name, before.id
            )
    ...
    async def on_member_remove(self, member):
        if member.guild == self.bot.verification_guild and member.bot:
            ...
            if get_category_id:
                ...
                for channel in category.channels:
                    if channel.type != discord.ChannelType.voice:
                        ...
                        reviewed_by = discord.utils.find(
                            lambda m: m.content.lower() in ["b!approve", "b!deny"], messages
                        )
                        for x in messages:
                            content = (
                                str(x.content)
                                if not x.embeds
                                else f"EMBED: {str(x.embeds[0].to_dict())}"
                                if not x.content
                                else f"CONTENT: {str(x.content)}\nEMBED: {str(x.embeds[0].to_dict())}"
                                if x.content and x.embeds
                                else "None"
                            )
                ...
                invited_by = await member.guild.audit_logs(
                    limit=2, action=discord.AuditLogAction.bot_add, before=category.created_at
                ).flatten()
                ...
                if x:
                    for user in x:
                        ...
                        if user["denied"]:
                            ...
                            await self.bot.pool.execute(
                                "DELETE FROM main_site_auditlogaction WHERE bot_id=$1", bot_unique_id
                            )
                            await self.bot.pool.execute("DELETE FROM main_site_bot WHERE id=$1", bot_id)
                            return
                        bots = " \n".join([f"{user['username']} (<@{user['id']}>)"])
                        ...
                        if user["id"] in self.test_categories.keys():
                            ...
                            for x in testing_category.text_channels:
                                try:
                                    await x.send(
                                        f"The owner of the bot ({user['username']} (<@{user['id']}>)) "
    ...
    async def check_join(self):
        ...
        for bot in bots:
            ...
            if not b:
                ...
                embed.add_field(name=bot["username"], value=invite)
                await channel.send(embed=embed)
    ...
    async def update_statuses(self):
        ...
        for bot in bots:
            ...
            if member_instance is None:
                break
            await self.bot.pool.execute(
                "UPDATE main_site_bot SET status = $1 WHERE id = $2", str(member_instance.status), bot["id"]
            )
                ...
                if int(testing_hours) >= 2:
                    ...
                    if ovm:  # user overwrites, hold command is used.
                        ...
                        invited_by = await bot_member.guild.audit_logs(
                            limit=2, action=discord.AuditLogAction.bot_add, before=category_created_at.flatten()
                        )
