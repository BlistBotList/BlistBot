...
from utils.flags import RestartFlags, ChangeVotesFlags
...
class Admin(commands.Cog):
    ...
    @commands.is_owner()
    @commands.command()
    async def restart(self, ctx, *, arguments: RestartFlags):
        """
...
""",
    ...
    async def avatars(self, ctx):
        ...
        for bot in bots:
            ...
            if bot_object:
                await self.bot.pool.execute(
                    "UPDATE main_site_bot SET avatar_url = $1 WHERE id = $2", str(bot_object.avatar_url), bot_object.id
                )
    ...
    async def changevotes(self, ctx, *, arguments: ChangeVotesFlags):
        """
        Change a bot or server's vote counts. You can choose from `--monthly`, `--total` or `--both`.

        **Arguments**:

        \u2022 id: - Can be a server id or bot id that is on the list. Make sure to use `list: server` if its a server id. Defaults to bot.
        \u2022 (+/-)amount: - Amount of votes to add (+) or remove(-) with the sign infront.
        \u2022 type: - This must be one of: `month, total, both`. Defaults to `both`.
        \u2022 list: - This must be one of: `bot, server`. Defaults to `bot`.

        **Example:** `b!changevotes id: 716713266683969626 amount: -10 type: month`
        ...
        add_or_remove = arguments.amount[0]
        amount = arguments.amount[1:]

        if add_or_remove not in ("+", "-"):
            return await ctx.send(f"amount must startwith + or -.")

        if not amount.isdigit():
            return await ctx.send(f"amount must be a number.")
        else:
            amount = int(amount)
        ...
        try:
            if arguments._type == "month":
                the_query = query["monthly"].format(
                    table=arguments._list,
                    remove_add=add_or_remove,
                    the_count=int(amount),
                    the_id=arguments._id,
                )
                await self.bot.pool.execute(the_query)
                return await ctx.send(f"{arguments.amount} to monthly_votes for {arguments._id}")

            elif arguments._type == "total":
                ...
                await self.bot.pool.execute(the_query)
                return await ctx.send(f"{arguments.amount} to total_votes for {arguments._id}")

            elif arguments._type == "both":
                ...
                await self.bot.pool.execute(the_query)
                return await ctx.send(f"{arguments.amount} to monthly_votes and total_votes for {arguments._id}")
    ...
    async def deny(self, ctx, suggestion: int, *, reason=None):
        ...
        m = await ch.fetch_message(fetch["message"])
        embed = discord.Embed(
            title=f"#{fetch['id']} Suggestion | Denieds",
            color=discord.Color.red(),
    ...
    async def strike(self, ctx, staffmember: discord.Member, strikes: int, *, reason):
        ...
        e.set_footer(text="blist.xyz", icon_url=ctx.guild.icon.url)
