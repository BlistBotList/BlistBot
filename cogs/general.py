...
""")
    ...
    async def botinfo(self, ctx, *, bot: discord.Member):
        ...
        if b['invite_url'] == '':
            invite = f"[Click Here](https://discordapp.com/api/oauth2/authorize?client_id={bot.id}&permissions=0&scope=bot)"
        else:
            invite = f"[Click Here]({b['invite_url']})"

        embed = discord.Embed(title=f"{bot.name}#{bot.discriminator}", description=f"""
