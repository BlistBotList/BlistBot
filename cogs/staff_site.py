...
class Staff(commands.Cog):
    ...
    async def queue(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="There are no bots in the queue",
        ...
        events_cog: Events = self.bot.get_cog("Events")  # type: ignore
        test_categories = events_cog.test_categories
        listed_bots = []
        for x in bots:
    ...
    async def approve(self, ctx, *, bot: discord.Member):
        ...
        if bots["uses_slash_commands"] is True:
            invite_scopes = ("bot", "applications.commands")
        invite = (
            str(discord.utils.oauth_url(bot.id, guild=self.bot.main_guild, scopes=invite_scopes))
            + "&disable_guild_select=true"
        )
    ...
    async def deny(self, ctx, bot: Union[discord.Member, discord.User]):
        ...
        try:
            ...
            await owner.send(
                f"Your bot `{bot}` with ID: {bot.id} was denied for the following reason: {reason}.\nSee more here: {msg.jump_url}"
            )
        except (discord.Forbidden, AttributeError):
            pass
        try:
    ...
    async def translate(self, ctx, *, arguments: TranslateFlags):
        ...
        **Optional Arguments**:
        --text/t | the text to translate. Example: `b!translate --t cool`
        --target/from | translate text to x language. Example: `b!translate --t cool --target en`
        --source/to | translate text from x language. Example: `b!translate --text cool --source nl`
        """
        message = " ".join(arguments.text)
        try:
            translated = self.translator.translate(message, dest=arguments.target, src=arguments.source)
        except ValueError:
            return await ctx.send(
