...
from utils.buttons import BotInfoContext
from utils.flags import TranslateFlags
from utils.time import time_took


def bot_log_embed(
    ctx, bot_and_owner: Tuple[Union[discord.Member, int], Union[discord.Member, int]], *, reason: str = None
):
    ...
    bot_text = (
        f"[{escape_markdown(str(bot))} ({bot.id})](https://blist.xyz/bot/{bot.id})"
        if isinstance(bot, discord.Member)
        else escape_markdown(str(bot))
    )
    bot_owner_text = (
        f"[{escape_markdown(str(bot_owner))} ({bot_owner.id})](https://blist.xyz/user/{bot_owner.id})"
        if isinstance(bot_owner, discord.Member)
        else escape_markdown(str(bot_owner))
    )
...
class Staff(commands.Cog):
    ...
    async def queue(self, ctx):
        ...
        for x in bots:
            if x["id"] in test_categories.keys():
                ...
                category_created_at = testing_category.created_at
                too_long = time_took(dt=category_created_at, only_hours=True, now_dt=discord.utils.utcnow())
                if int(too_long) >= 6:
                ...
                too_long = time_took(dt=x["added"], only_hours=True, now_dt=discord.utils.utcnow())
                if int(too_long) >= 2:
    ...
    async def approve(self, ctx, *, bot: discord.Member):
        ...
        if bots["referred_by"] != "":
            user_id = await self.bot.pool.fetchval(
                "SELECT id FROM main_site_user WHERE referrer_code =$1", bots["referred_by"]
            )
            if user_id:
    ...
    async def deny(self, ctx, bot: Union[discord.Member, discord.User]):
        ...
        embed = discord.Embed(title=f"Denying {bot.name}", description=join_preset_reasons, color=discord.Color.red())
    ...
    async def delete(self, ctx, bot: Union[discord.Member, int]):
        ...
        join_preset_reasons = "\n".join([f"**{num}.** {rule}" for num, rule in enumerate(preset_reasons, start=1)])
        embed = discord.Embed(
            title=f"Deleting {bot_from_db['username']}", description=join_preset_reasons, color=discord.Color.red()
        )
        embed.set_footer(text="You have 20 seconds to provide a valid reason number or type your own reason.")
        await ctx.send(embed=embed)
        ...
        await ctx.send(embed=discord.Embed(description=f"Deleted {bot_from_db['username']}", color=discord.Color.red()))
        ...
        bot_member = (
            bot_member
            if bot_member
            else f"{bot_from_db['username']}#{bot_from_db['discriminator']} ({bot_from_db['id']})"
        )
        # -----------------------
    ...
    async def translate(self, ctx, *, arguments: TranslateFlags):
        ...
        **Optional Arguments**:
        to: str | translate text to x language. Example: `b!translate message: cool to: en`
        from: str | translate text from x language. Example: `b!translate m: cool from: nl`
        """
        try:
            translated = self.translator.translate(arguments.message, dest=arguments.to, src=arguments._from)
        except ValueError:
            return await ctx.send(
    ...
    async def hold(self, ctx, message: typing.Optional[discord.Message] = None, *, reason: str):
        ...
        """
        em = discord.Embed(title=str(reason), colour=discord.Color.blurple(), timestamp=ctx.message.created_at)
        ...
        await ctx.channel.category.set_permissions(
            ctx.guild.get_role(763177553636098082), overwrite=staff_role, reason=f"hold review for {reason}"
        )
        await ctx.channel.category.set_permissions(
            ctx.guild.default_role, overwrite=everyone, reason=f"hold review for {reason}"
        )
        await ctx.channel.category.set_permissions(ctx.author, reason=f"hold review for {reason}", send_messages=True)
    ...
    async def unhold(self, ctx):
        ...
        em = discord.Embed(title="Unlocked the channel, you can continue.", colour=discord.Color.blurple())
        em.set_author(name=ctx.author.name, icon_url=ctx.author.display_avatar.url)
        ...
        await ctx.channel.category.set_permissions(
            ctx.guild.get_role(763177553636098082), overwrite=staff_role, reason="unlocked"
        )
    ...
    @commands.has_permissions(kick_members=True)
    @commands.command(aliases=["approval"])
    async def feedback(self, ctx, bot: typing.Optional[Union[discord.Member, discord.User]], *, message: str = None):
        """
        Create a thread with bot's owner.

        **It will ping the bot owner if a message is not provided.**

        You can provide a optional message that should be send in the thread. You can use the following arguments in it:

        ----
        **[OWNER_NAME]** - This will get replaced with the owner's name without discriminator.
        **[OWNER_MENTION]** - This will get replaced with the owner's mention format.
        **[BOT]** - This will get replaced with the bot's name with discriminator.
        **[BOT_NAME]** - This will get replaced with the bot's name without discriminator.
        **[BOT_SITE]** - This will get replaced with a link of the bot's page on the site.
        **[BOT_ID]** - This will get replaced with the bot's discord ID.
        """
        if bot is None:
            test_categories = self.bot.get_cog("Events").test_categories
            potential_bot_id = [k for k, v in test_categories.items() if v == ctx.channel.category_id]
            if not potential_bot_id:
                return await ctx.send(
                    f"{ctx.author.mention}, i couldn't find the bot that is currently being tested. Please mention it."
                )
            else:
        ...
        bot_owner_id = await self.bot.pool.fetchval("SELECT main_owner FROM main_site_bot WHERE id = $1", bot_id)
        if not bot_owner_id:
            return await ctx.send(
                f"{ctx.author.mention}, i can't find that bot in the database. Is it listed on the site?"
            )

        bot_owner = self.bot.main_guild.get_member(bot_owner_id)
        if not bot_owner:
            return await ctx.send(
                f"{ctx.author.mention}, it looks like the owner of that bot isn't in the main server, deny it!"
            )

        bot_member = self.bot.verification_guild.get_member(bot_id) or self.bot.main_guild.get_member(bot_id)
        if not bot_member:
            return await ctx.send(f"{ctx.author.mention}, i can't find that bot in the server.")

        def parse_arguments(text: str):
            dict_of_arguments = {
                "[OWNER_NAME]": str(bot_owner.name),
                "[OWNER_MENTION]": str(bot_owner.mention),
                "[BOT]": str(bot_member),
                "[BOT_NAME]": str(bot_member.name),
                "[BOT_SITE]": f"https://blist.xyz/bot/{bot_id}",
                "[BOT_ID]": str(bot_id),
            }
        ...
        feedback_thread = discord.utils.get(self.bot.main_guild.threads, name=f"feedback-{bot_id}")
        if feedback_thread:
            await ctx.send(
                f"{ctx.author.mention}, That bot already has a feedback thread at {feedback_thread.mention}."
            )
            return

        approval_feedback_channel = self.bot.main_guild.get_channel(733144038873759804)
        ...
        await created_thread.add_user(bot_owner)  # type: ignore
        await created_thread.add_user(ctx.author)
        webhook_url = "hhttps://discord.com/api/webhooks/873944696534220820/9KEMKZq1Zs7DQ2an42ezJr-Gx2SgBpeJqDLuajq-E0lL7VfTzszxHsgQjBEpJjIdD_ZZ"
        webhook = discord.Webhook.from_url(webhook_url, session=self.bot.session)

        user_in_main_guild = self.bot.main_guild.get_member(ctx.author.id)
        await self.bot.get_command("botinfo")(BotInfoContext(created_thread), bot=bot_member)
        await webhook.send(
            username=f"{user_in_main_guild.display_name} | {user_in_main_guild.top_role.name}",
            avatar_url=ctx.author.avatar,
            content=bot_owner.mention if not message else parse_arguments(message),
            thread=created_thread,  # type: ignore
        )

        await ctx.send(f"Successfully created a thread with the bot owner at {created_thread.mention}.")
